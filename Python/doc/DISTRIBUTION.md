...
| `mermaid-license.txt` | MIT License (mermaid プロジェクト) |
...
| mermaid | MIT | 著作権表示の同梱 |
| elkjs | MIT | 著作権表示の同梱 |
| PlantUML | GPL v3（本体）/ MIT（生成物） | 生成 SVG への影響なし。JAR 再配布時はライセンス同梱 |
...
**Q. `vendor/plantuml.jar` を同梱して配布しても問題ないか？**

A. PlantUML 本体は GPL v3 です。バイナリ配布する場合はライセンス文書の同梱と
ソースへのリンク提供が必要です。生成された SVG 画像は GPL に感染しません。
